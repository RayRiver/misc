local TestClass = class("TestPerformance", require("src.TestBase"))
...
function TestClass:onEnter()
    ...
    local function onIncreaseArmatures()
        ...
        for i = 1, 20 do
            ...
            addArmatureToParent(self, armature)
        end
        refreshTitle()
    end
    ...
    local function onDecreaseArmatures()
        ...
        for i = 1, 20 do
            removeArmatureFromParent(self, ARMATURE_PERFORMANCE_TAG + nArmatureCount)
            nArmatureCount = nArmatureCount - 1
        end
        refreshTitle()
    end
